    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`https://tourism-management-server-pearl.vercel.app/spots/${id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
                    .then(data => {
    ...
    return (
        <div>
            ...
            <div className="overflow-x-auto px-5">
                {spots.length === 0 ? (
                    <p className="text-center mt-4">No spots added yet.</p>
                ) : (
                            ...
                            {spots.map((spot) =>
                                user.email === spot.email && (
                                    <tr key={spot._id}>
                                        ...
                                        <td>
                                            <button onClick={() => handleDelete(spot._id)} className="btn btn-error">
                                                X Delete
                                            </button>
                                        </td>
                                    </tr>
